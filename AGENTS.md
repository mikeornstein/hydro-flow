...
1-D flow-network modeling in the browser. The engine lives in `src/engine`, the canvas in `src/ui`. Research lives in `docs/MACROFLOW_RESEARCH.md`. Workflows live in `docs/WORKFLOWS_AND_ACCEPTANCE.md`. Verification cases live in `docs/VERIFICATION_CASES.md`. Open physics decisions live in `docs/OPEN_PHYSICS_DECISIONS.md`. The project file schema is `docs/schema/hydroflow.project.schema.json` and matches `src/engine/types.ts`.
